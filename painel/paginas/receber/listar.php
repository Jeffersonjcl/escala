...
HTML;
	for ($i = 0; $i < $total_reg; $i++) {
		...
		$classe_venda = 'ocultar';
		$total_itens_venda = 0;
		if ($referencia == 'Venda') {
			//total de itens venda
			$res2 = $pdo->query("SELECT * from itens_venda where id_venda = '$id' order by id asc");
			$dados2 = $res2->fetchAll(PDO::FETCH_ASSOC);
			$linhas2 = count($dados2);
			for ($i2 = 0; $i2 < $linhas2; $i2++) {
				$total_it = $dados2[$i2]['quantidade'];
				$total_itens_venda += $total_it;
			}
		...
		$mostrar_os = 'ocultar';
		if ($referencia == 'Serviço' || $referencia == 'Nova OS') {
			$mostrar_os = '';
		}
		...
		<i class="fa fa-square {$classe_pago} mr-1"></i> {$descricao} <span class="{$classe_venda} text-primary"><small> <a href="#" onclick="modalItens('{$id}')">({$total_itens_venda} Itens)</a> </small></span>

		<form method="POST" class="" action="rel/os_class.php" target="_blank" style="display:inline-block">
			<input type="hidden" name="id" value="{$id_ref}">
			<button title="PDF da OS" class="{$mostrar_os}" style="background:transparent; border:none; margin:0; padding:0"><i class="fa fa-file-pdf-o text-danger" style="color:white;"></i></button>
		</form>
		...
		<td>
			...
			<a class="btn btn-info btn-sm {$ocultar}" href="#" onclick="editar('{$id}', '{$descricao}', '{$cliente}','{$valorF}','{$data_venc}','{$frequencia}','{$saida}','{$arquivo}', '{$obs}','{$data_pgto}')" title="Editar Dados"><i class="fa fa-edit"></i></a>

			<a class="btn btn-warning btn-sm" href="#" onclick="mostrar('{$id}', '{$descricao}', '{$nome_pessoa}', '{$tipo_pessoa}','{$valorF}','{$data_lancF}','{$data_vencF}','{$data_pgtoF}','{$nome_usu_lanc}','{$nome_usu_pgto}','{$nome_frequencia}','{$saida}','{$arquivo}','{$pago}','{$obs}','{$pix_pessoa}','{$tel_pessoa}')" title="Ver Dados"><i class="fa fa-info-circle"></i></a>
			...
			<div class="dropdown" style="display: inline-block;">                      
				<a class="btn btn-danger btn-sm {$ocultar_excluir}" href="#" aria-expanded="false" aria-haspopup="true" data-bs-toggle="dropdown" class="dropdown" title="Excluir Recebimento"><i class="fa fa-trash-can"></i></a>
	...
	<br>
	<div align="right"><span>Total Pendente: <span class="text-danger">{$total_pendentesF}</span></span> <span style="margin-left: 25px">Total Recebido: <span class="verde">{$total_pagoF}</span></span></div>

HTML;
} else {
	echo 'Não encontrei nenhum Recebimento para esta data ou período!';
}
...
<script type="text/javascript">
	...
	function mostrar(id, descricao, nome_pessoa, tipo_pessoa, valor, data_lanc, data_venc, data_pgto, usuario_lanc, usuario_pgto, frequencia, saida, arquivo, pago, obs, pix, tel) {
		...
		$('#pessoa_mostrar').text(nome_pessoa);
