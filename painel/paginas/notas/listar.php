...
$query = $pdo->query("SELECT * from $pagina WHERE data_entrega >= '$dataInicial' and data_entrega <= '$dataFinal' and status LIKE '$status' $sqlCliente order by id desc ");
...
HTML;
	...
	for ($i = 0; $i < $total_reg; $i++) {
		...
		<td>{$numero_nota}</td>
		<td width="15%">{$nome_cliente}</td>
		<td>{$nome_fornecedor}</td>
		<td>{$data_entregaF}</td>
		<td style="color: {$cor_status}; font-weight:bold">R$ {$valorF}</td>
		<td><a href="images/notas/{$nota}" target="_blank"><img class="hovv" src="images/notas/{$tumb_arquivo}" width="25px" height="25px"></a></td>
		<td><div style="color:#FFF; background:{$cor_status}; padding:4px; width:100%; text-align: center; font-size: 12px;">{$status}</div></a>

		<td>

			<div class="dropdown head-dpdn2" style="display: inline-block;"  id="aprovar_nota">
				<a class="btn btn-success btn-sm {$ocultarAprovar}" title="Aprovar Nota" href="#" aria-expanded="false" aria-haspopup="true" data-bs-toggle="dropdown" class="dropdown"><i class="fa fa-check-square"></i></a>
				<div  class="dropdown-menu tx-13">
					<div style="width: 240px; padding:15px 5px 0 10px;" class="dropdown-item-text">
						<p>Confirmar Aprovação? <a href="#" onclick="aprovar('{$id}')"><span class="text-verde">Sim</span></a></p>
					</div>
				</div>
			</div>
			
			<div class="dropdown head-dpdn2" style="display: inline-block;" id="editar_nota">
				<a class="btn btn-secondary btn-sm {$ocultarClienteEditar}" title="Permitir Edição da Nota" href="#" aria-expanded="false" aria-haspopup="true" data-bs-toggle="dropdown" class="dropdown"><i class="fa fa-check-square"></i></a>
				<div  class="dropdown-menu tx-13">
					<div style="width: 240px; padding:15px 5px 0 10px;" class="dropdown-item-text">
						<p>Permitir Edição da Nota? <a href="#" onclick="abrirEditar('{$id}')"><span class="text-verde">Sim</span></a></p>
					</div>
				</div>
			</div>

			<a class="btn btn-info btn-sm {$ocultarEditarExcluir}" href="#" onclick="editar('{$id}','{$numero_nota}','{$data_entrega}','{$valorF}','{$fornecedor}','{$cliente}','{$obs}','{$nota}')" title="Editar Dados"><i class="fa fa-edit"></i></a>
			...
			<!-- excluir -->
			<div class="dropdown" style="display: inline-block;">                      
				<a class="btn btn-danger btn-sm {$ocultarEditarExcluir}" href="#" aria-expanded="false" aria-haspopup="true" data-bs-toggle="dropdown" class="dropdown"title="Excluir Orçamento"><i class="fa fa-trash-can"></i></a>
				<div  class="dropdown-menu tx-13">
					<div style="width: 240px; padding:15px 5px 0 10px;" class="dropdown-item-text">
...
HTML;
	...
	echo 'Não encontrei nenhuma Nota para esta Data ou Período!';
}
...
<script type="text/javascript">
	...
	function editar(id, numero_nota, data_entrega, valor, fornecedor, cliente, obs, nota) {
		...
		if (resultado[1] === 'pdf') {
			...
			$('#target').attr('src', 'images/notas/' + nota);
			return;
		}
	...
	function mostrar(id, numero_nota, data_entrega, valor, fornecedor, cliente, usu_cad, data_cad, hora_cad, obs, nota) {
		...
		$('#link_notas').attr('href', 'images/notas/' + nota);
		...
		if (resultado[1] === 'pdf') {
			$('#nota_dados').attr('src', "images/pdf.png");
			return;
		} else if (resultado[1] === 'rar' || resultado[1] === 'zip') {
			$('#nota_dados').attr('src', "images/rar.png");
			return;
		} else if (resultado[1] === 'doc' || resultado[1] === 'docx') {
			$('#nota_dados').attr('src', "images/word.png");
			return;
		} else if (resultado[1] === 'xlsx' || resultado[1] === 'xlsm' || resultado[1] === 'xls') {
			$('#nota_dados').attr('src', "images/excel.png");
			return;
		} else if (resultado[1] === 'txt') {
			$('#nota_dados').attr('src', "images/txt.png");
			return;
		} else if (resultado[1] === 'xml') {
			$('#nota_dados').attr('src', "images/xml.png");
			return;
		} else {
			$('#nota_dados').attr('src', 'images/notas/' + nota);
			return;
		}
